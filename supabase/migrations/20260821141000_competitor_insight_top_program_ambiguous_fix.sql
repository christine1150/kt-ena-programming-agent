-- 버그 수정(2026-08-21, 직전 마이그레이션 20260821140000 회귀): get_competitor_insight_report는
-- language plpgsql이라 RETURNS TABLE에 선언한 컬럼명(competitor_name 등)이 함수 본문 전체에서
-- 암묵적 변수로도 보인다 — 새로 추가한 top_program CTE의 "distinct on (competitor_name) ...
-- order by competitor_name"이 별칭 없이 컬럼명만 써서 "column reference competitor_name is
-- ambiguous"(42702) 런타임 오류를 냈다(직접 RPC 호출로 재현 확인, p_date_from을 준 멀티데이
-- 조회에서 100% 발생 — 이 CTE가 그 경우에만 실행되는 top_program_agg를 참조하기 때문). 이
-- 세션에서 이미 한 번 겪었던 것과 같은 함정(OUT 파라미터 섀도잉)인데 새 CTE에 다시 반영을 놓쳤다.
-- 테이블 별칭을 명시해 고친다.
...
$$;
comment on function get_competitor_insight_report is 'Page 2 COMPARED WITH? 보고서용: 등록 경쟁채널을 순위 순으로, 최근 12주 평균 대비 등락과 기간 내 최고 성적 프로그램(제목+그 기간 평균 시청률+방영 횟수)까지 제공. 최고 성적 프로그램은 프로그램명 단위로 그 기간 모든 방영분을 평균 내어 고른다(단일 방영 스파이크 방지). 단일 일자 조회만 방영 시각을 함께 반환. 자사 타깃 라벨이 competitor_ratings 표기와 다르면 그 채널 경쟁채널들이 실제로 쓴 target_id로 자동 대체한다. (2026-08-21 수정: top_program CTE의 컬럼 참조가 RETURNS TABLE의 동명 컬럼과 겹쳐 ambiguous 오류가 나던 것을 별칭으로 고침.)';
