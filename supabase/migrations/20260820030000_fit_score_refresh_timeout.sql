-- 회고 리뷰(2026-08-20) 중 브라우저로 직접 확인한 버그: Page 2 기간 설정에서 MART가 아직
-- 없는 날짜(예: MoM=1개월 전)를 고르면 /api/scheduling/fit-score가
-- "canceling statement due to statement timeout"로 500을 반환했다. refresh_fit_score_mart()가
-- 채널 6개 × (get_target_achievement + get_target_affinity 최대 4회 + get_competitive_pressure
-- 3회 + 슬롯/프로그램/Flow 집계)를 한 번에 도는 무거운 콜드 계산이라, 이미 계산해둔 날짜가
-- 아닌 새 날짜를 그 자리에서 계산할 때 기본 statement_timeout(역할/커넥션 풀 설정)을 넘길 수
-- 있다. 이 함수 실행 트랜잭션에 한해서만(set_config의 세 번째 인자 true = 트랜잭션 로컬) 타임아웃을
-- 넉넉히 늘려 콜드 계산이 안전하게 끝나도록 한다 — 매일 자동 갱신되는 정상 경로(캐시 hit)에는
-- 영향 없음.
...
begin
  -- 콜드 계산(아직 캐시되지 않은 as_of_date)이 기본 statement_timeout보다 오래 걸릴 수 있어,
  -- 이 트랜잭션에 한해서만 5분으로 늘린다(실제로 겪은 500 에러: "canceling statement due to
  -- statement timeout").
...
$$;
comment on function refresh_fit_score_mart is 'Fit Score MART 전체 재계산(as_of_date 하루치, 최근 12주 데이터 기준). 관리자가 필요할 때 호출하거나, 매일 Nielsen 업로드 이후 실행한다. 콜드 계산용으로 이 트랜잭션 한정 statement_timeout을 5분으로 늘림(20260820030000).';
