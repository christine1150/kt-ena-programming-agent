-- 사용자 지시(2026-08-26): "왕자와 거지는 ENA Play가 동시 방송을 하는 것을 알 수 있어요. 동시
-- 방송을 할 경우에는 동시 방송 성적을 가장 먼저 올려주시고, 이후 직후재방이 있을 경우에만
-- 직후재방을 언급해주세요... 이 패턴을 기억해서 이후에도 반영해주세요. 브리핑이나 보고서도
-- 마찬가지에요."
--
-- 실측 확인(2026-08-24 Nielsen): 왕자와거지는 ENA 23:21:04 본방, ENA Play 23:14:58 — ENA Play가
-- ENA 본방보다도 먼저 시작해 사실상 동시방송이다(직후재방처럼 본방 종료 후가 아님). 그런데
-- 20260826060000 마이그레이션이 사용자가 전달한 시트를 옮겨적을 때 ENA Play를 rerun_channel_id
-- (직후재방)에 잘못 넣었다 — 원 시트의 "동시방송" 칸 값이었는데 "직후 재방" 칸으로 잘못 이관됨.
-- 이번에 바로잡는다: 왕자와거지의 ENA Play 관계를 simulcast_channel_id로 옮기고 rerun_channel_id는
-- 비운다(신병4:사보타주의 ENA Drama는 본방 종료 3분 뒤 시작 — 실제로 직후재방이 맞아 그대로 둔다).
--
-- 그리고 이제부터 동시방송(simulcast)과 직후재방(rerun)을 서로 다른 개념으로 분리해서 계산한다
-- (지금까지는 rerun_channel_id가 비어 있을 때만 simulcast_channel_id로 대체하는 fallback이었는데,
-- 이러면 진짜 동시방송이 "직후재방"이라는 잘못된 라벨로 보인다). simulcast는 본방 시작 시각과
-- 가까운(±30분) 그 채널의 실제 방영분을 매칭하고(선행 재방·본방 매칭과 같은 원리, 직후재방처럼
-- "본방 종료 후"라는 시간 제약이 없음), rerun은 기존 그대로 "본방 종료 후 그 채널 첫 프로그램"을
-- 본다. 두 필드는 이제 항상 서로 다른 채널을 가리키므로(데이터 입력 규칙상 한 프로그램이 같은
-- 채널을 동시방송이면서 직후재방으로 이중 등록하지 않음) 중복 계산 우려는 없다.
update featured_content fc
set simulcast_channel_id = (select id from channels where code = 'ENA_PLAY'),
    rerun_channel_id = null
from programs p
where p.id = fc.program_id
  ...
  and fc.rerun_channel_id = (select id from channels where code = 'ENA_PLAY');
...
as $$
  ...
  -- 제약 없이, 본방 시작 시각과 가까운(±30분, 편성 오차 여유) 그 채널 실제 방영분을 매칭한다.
  left join lateral (
...
$$;
comment on function get_original_content_daily is 'Page 1 주요 콘텐츠 리뷰: featured_content(주요 콘텐츠 관리) 중 오늘 요일·시각에 편성된 항목을 실제 방영 데이터(±10분 매칭)와 연결. 동시방송(simulcast_channel_id, 본방과 가까운 시각 ±30분 매칭)과 직후재방(rerun_channel_id, 본방 종료 후 그 채널 첫 프로그램)을 서로 다른 개념으로 분리 계산(2026-08-26). 첫 방송일자/종영일 범위 밖 프로그램 자동 제외, 같은 슬롯 중복은 실제 방영 프로그램명과 일치하는 항목 우선으로 1건만 남김. 분류·표시명은 featured_content에서 바로 가져온다.';
