-- 개발 단위 17번(계속): Fit Score MART 재계산 함수.
-- Program×Target 단위로 6개 하위지표를 계산해 mart_scheduling_fit_score까지 채운다.
-- Claude나 API가 이 함수를 호출만 하고, 계산 자체는 전부 여기(SQL)에서 실행된다.
...
$$;
comment on function refresh_fit_score_mart is 'Fit Score MART 전체 재계산(as_of_date 하루치, 최근 12주 데이터 기준). 관리자가 필요할 때 호출하거나, 매일 Nielsen 업로드 이후 실행한다.';
