-- 사용자 지시(2026-08-25): Page 1 "주요 컨텐츠 리뷰" 제목을 "<프로그램명> N회 시청률 (타깃 및
-- 가구 하락, 동시간대 타깃 5위, 가구 3위)" 형태로 바꾸려면, 지금까지는 "가구 순위"(householdRank,
-- route.ts에서 get_competitor_program_overlap 결과 개수로 별도 계산)만 있고 "가구 시청률이
-- 전회 대비 올랐는지/내렸는지"는 없었다 — 타깃(수도권 2049 등) 쪽과 완전히 동일한 방식(같은
-- 슬롯 매칭 ±10분, 직전 방영 대비)으로 "전국 유료가구" 타깃 버전을 추가한다. 기존 컬럼·로직은
-- 전혀 건드리지 않고 3개 컬럼만 추가(matched_household_rating/prior_household_rating/
-- household_rating_change_pct) — 반환 타입이 늘어나 drop 후 재생성 필요.
...
$$;
comment on function get_original_content_daily is 'Page 1 Original 리포트(평일): 화이트리스트를 실제 방영 데이터(±10분 매칭)와 연결. 직후재방·선행 재방·당일 자체 재방·직전 방영 대비·회차 번호·도달율·연령대별 상위 5개·가구(전국 유료가구) 타깃 시청률과 그 전회 대비 등락(2026-08-25 추가)까지 포함.';
