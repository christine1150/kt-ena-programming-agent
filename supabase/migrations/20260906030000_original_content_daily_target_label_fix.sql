-- 자체 발견 버그 수정(2026-09-06, 중대) — get_original_content_daily의 본방/직재방/
-- 선행재방/자체재방/전회 대비 5개 lateral join이 전부 target_id(타깃) 필터 없이
-- "least(시간차) asc limit 1"만으로 행을 고르고 있었다.
--
-- Nielsen 데이터는 같은 채널·같은 start_time에 타깃(수도권 2049/2039/남녀 연령대/
-- 전국 유료가구 등 십수 개)별로 여러 행이 동시에 존재한다(실측: ENA 2026-09-05
-- 19:50 방영분에 17개 타깃 행, ENA_PLAY 23:59:55 재방에도 18개 타깃 행). 그런데
-- "least(시간차) asc limit 1"은 이 십수 개 행이 전부 시간차 0으로 동률이라 그중
-- 어느 타깃이 뽑힐지 SQL 표준상 보장되지 않는다(정렬 키에 타깃이 없으면 결과가
-- 힙 스캔 순서 등 구현 세부사항에 좌우됨) — 실측으로 실제 오류를 확인했다:
-- 짐쌀라비움 ENA Play 직재방(2026-09-05 23:59:55)의 "수도권 2049" 실제 시청률은
-- 0.01544인데, rr 서브쿼리는 같은 시각의 "수도권 2039"(우연히 0) 등 다른 타깃
-- 행을 집어 rerun_rating을 0으로 잘못 반환하고 있었다 — 화면에 "직재방 시청률
-- 0%"로 나온 것 자체가 이 버그의 증상이었다.
--
-- 고친 부분: whitelist CTE가 이미 계산해 두는 program_target_label(그룹 A는
-- "수도권 2049", 그룹 B는 "전국 유료가구")을 matched(m)/rr/pre/self_r/prior
-- 5개 lateral join에 명시적으로 타깃 조인+필터로 추가해 항상 정확히 그 타깃의
-- 행만 고르게 한다. sim(동시방영) 조회는 이미 m.matched_target_id_val로 정확한
-- target_id를 지정하고 있어(이번에 matched가 고쳐지면 자동으로 함께 정확해짐)
-- 별도 수정이 필요 없다. age(연령대 분해)·hh/hh_prior(가구)는 원래부터 각자
-- 목적에 맞는 타깃 조건을 갖고 있어 그대로 둔다. 반환 컬럼·시그니처는 동일.
