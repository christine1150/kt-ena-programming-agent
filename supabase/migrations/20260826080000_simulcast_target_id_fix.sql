-- 후속 수정(2026-08-26): 20260826070000이 적용된 후 발견 — sim(동시방송) lateral join이
-- target_id를 걸지 않아서, 프로그램 하나에 타깃별로 여러 ratings 행이 있을 때(수도권 2049 /
-- 수도권 2039 / 전국 유료가구 / 연령대별...) 어느 것을 매칭할지 결정 불가능했다(전부 같은
-- start_time이라 시간차 0으로 동률 — LIMIT 1이 사실상 임의의 타깃을 골랐다. 실측 확인: 왕자와거지
-- 동시방송 시청률이 수도권2049(0.03909) 대신 수도권2039(0.06175)로 잘못 나옴). matched_target_id_val
-- (본방 시청률이 쓴 것과 같은 target_id)로 sim도 고정해 본방과 동일 타깃끼리 비교되도록 한다.
-- (매 마이그레이션은 신규 추가만 한다는 원칙에 따라 070000 파일을 다시 고치지 않고 여기서
-- 함수만 한 번 더 재생성 — 본문은 070000과 완전히 동일, DB에 실제 반영만 새로 함.)
...
$$;
comment on function get_original_content_daily is 'Page 1 주요 콘텐츠 리뷰: featured_content(주요 콘텐츠 관리) 중 오늘 요일·시각에 편성된 항목을 실제 방영 데이터(±10분 매칭)와 연결. 동시방송(simulcast_channel_id, 본방과 가까운 시각 ±30분 매칭, 본방과 같은 target_id로 고정)과 직후재방(rerun_channel_id, 본방 종료 후 그 채널 첫 프로그램)을 서로 다른 개념으로 분리 계산(2026-08-26). 첫 방송일자/종영일 범위 밖 프로그램 자동 제외, 같은 슬롯 중복은 실제 방영 프로그램명과 일치하는 항목 우선으로 1건만 남김. 분류·표시명은 featured_content에서 바로 가져온다.';
