...
const KNOWN_CATEGORIES = ["오리지널", "오리지널 드라마", "오리지널 예능", "독점 예능", "브랜디드", "구매 예능", "구매 드라마"];
  ...
  return (
    <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-zinc-100">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-zinc-900">주요 콘텐츠 관리</h2>
          <p className="text-sm text-zinc-500">
            Channel Master 업로드로 자동 반영된 항목 + 직접 추가한 항목을 함께 관리합니다.
          </p>
        </div>
        <button
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-t border-zinc-100">
                  ...
                  <td className="py-1.5 text-zinc-800">{item.programs?.canonical_name ?? "—"}</td>
