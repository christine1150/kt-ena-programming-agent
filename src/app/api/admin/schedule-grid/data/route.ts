...
// 재지시(2026-09-20): "기본적으로 DB 기반으로 구성하되, 업로드된 편성표가 매치되는 회차나
// 부제가 있으면 그것만 덧붙이는" — 기본 동작 자체가 DB 재구성 + 업로드 메타데이터 보강으로
// 바뀌었다(getScheduleGridRows). ?view=upload로 업로드 원본 그대로 보기로 전환 가능
// (forceUpload). "채널 그라데이션이 더 잘 비교되게, 연간 채널 평균보다 높은 칸을 잘 보이게" —
// 이 채널의 연초~오늘 누적 평균 시청률(Page 1 히어로 카드와 같은 계산)을 함께 내려줘 화면이
// 고정 기준선으로 색·굵기를 정할 수 있게 한다.
