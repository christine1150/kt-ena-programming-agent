...
export async function GET(request: Request) {
  ...
  const { data: trend, error: trendError } = await supabase.rpc("get_rating_trend_summary", {
    p_channel_code: channel.code,
    p_target_label: matchedTargetLabel,
    p_as_of_date: asOfDate,
  });
  if (trendError) {
    return NextResponse.json({ ok: false, message: trendError.message }, { status: 400 });
  }

  // HOW DEEPLY? / 02~26시 시간대별 그래프 — 프로그램 단위 데이터가 필요해서, 타깃 라벨을
  // 타깃상세 시트 표기로 바꿔서 조회한다 (src/lib/targetResolution.ts 참고).
  // 기간 설정(사용자 지시): 두 함수 모두 원래 하루만 됐는데 dateFrom~dateTo 범위를 그대로
  // 넘겨 기간 전체를 집계하도록 확장했다(dateFrom=dateTo면 기존과 동일).
  const programTargetLabel = resolveProgramLevelTargetLabel(channel.primary_target);
  let { data: hourlyPattern } = await supabase.rpc("get_hourly_rating_pattern", {
    p_channel_code: channel.code,
  ...
  let { data: hourlyProgramTitles } = await supabase.rpc("get_hourly_program_titles", {
    p_channel_code: channel.code,
    p_target_label: programTargetLabel,
    p_date_from: dateFrom,
    p_date_to: dateTo,
  ...
  // 알려준다(화면에 "최근 프로그램 데이터 기준(8/17)"처럼 표시할 수 있게).
  ...
  if (dateFrom === dateTo && (!hourlyPattern || hourlyPattern.length === 0)) {
    ...
    if (fallbackDateRow?.broadcast_date) {
      ...
      const { data: fallbackPattern } = await supabase.rpc("get_hourly_rating_pattern", {
        p_channel_code: channel.code,
        p_target_label: programTargetLabel,
        p_date_from: hourlyEffectiveDate,
        p_date_to: hourlyEffectiveDate,
      });
      hourlyPattern = fallbackPattern;
      const { data: fallbackTitles } = await supabase.rpc("get_hourly_program_titles", {
        p_channel_code: channel.code,
        p_target_label: programTargetLabel,
        p_date_from: hourlyEffectiveDate,
        p_date_to: hourlyEffectiveDate,
  ...
  // 사용자 지시(2026-08-20): "시간대별 그래프에서 각 채널의 최근 12주 시간대별 평균 시청률을
  // 연한 색으로 꺾은선 그래프로 그려서 기준점을 보여줄 것" — 선택 기간과 별개로 dateTo 기준
  // 직전 84일 고정 윈도우를 같은 함수로 한 번 더 조회한다(신규 SQL 불필요, 기존 함수 재사용).
  const { data: hourlyBaselinePattern } = await supabase.rpc("get_hourly_rating_pattern", {
    p_channel_code: channel.code,
    p_target_label: programTargetLabel,
    p_date_from: addDaysStr(dateTo, -83),
    p_date_to: dateTo,
  });
  ...
  // 최근 12주 평균 대비, 기간 중 최고/최저일. 단일 일자(dateFrom=dateTo)에서도 그대로 동작한다.
  const { data: periodReportData } = await supabase.rpc("get_rating_period_report", {
    p_channel_code: channel.code,
    p_target_label: matchedTargetLabel,
    p_date_from: dateFrom,
    p_date_to: dateTo,
  ...
  const periodReport = periodReportData?.[0] ?? null;

  // COMPARED WITH? 재설계(사용자 지시) — Competitive Pressure 숫자 대신, 등록 경쟁채널을 순위
  // 높은 순으로 나열하고 최근 12주 평균 대비 등락 + 최고 성적 프로그램(시간대)을 함께 제공.
  // 기간 선택 시 p_date_from을 넘기면 "오늘"이 "기간 평균"으로 집계된다(baseline은 기간 이전으로
  // 겹치지 않게 계산). 동시간대 겹치는 경쟁 프로그램 비교(overlap)는 여러 날을 합치면 의미가
  // 흐려져(같은 시간대에 여러 날의 프로그램이 뒤섞여 표시됨) 의도적으로 dateTo 하루만 유지한다
  // (화면에 그 사실을 명시). 시장 전체 TOP N은 기간 전체에서 뽑도록 확장.
  const { data: competitorInsightReport } = await supabase.rpc("get_competitor_insight_report", {
    p_channel_code: channel.code,
    p_target_label: matchedTargetLabel,
    p_as_of_date: dateTo,
    p_date_from: dateFrom,
  });
  const { data: overlapData } = await supabase.rpc("get_competitor_program_overlap", {
    p_channel_code: channel.code,
    p_target_label: programTargetLabel,
    p_as_of_date: dateTo,
  });
  const { data: topProgramsData } = await supabase.rpc("get_competitor_top_programs", {
    p_channel_code: channel.code,
    p_as_of_date: dateTo,
    p_limit: 5,
    p_date_from: dateFrom,
  ...
  // WHY? — 원인 추적(Root-Cause 참고 분석). matchedTargetLabel(채널 단위 랭킹 데이터) 기준.
  // 3일 연속 하락처럼 "그 시점까지의 추세"를 보는 지표라 선택 기간의 마지막 날짜(dateTo)를
  // 기준일로 그대로 쓴다(기간 선택과 무관하게 원래도 trailing window 설계).
  const { data: rootCauseData } = await supabase.rpc("get_root_cause_alert", {
    p_channel_code: channel.code,
    p_target_label: matchedTargetLabel,
    p_as_of_date: dateTo,
  });
  const rootCauseAlert = rootCauseData?.[0] ?? null;

  // OPPORTUNITY? — 기회 탐지(Opportunity Alert). 같은 매칭 타깃 라벨 기준, dateTo 기준일.
  const { data: opportunityAlertData } = await supabase.rpc("get_opportunity_alert", {
    p_channel_code: channel.code,
    p_target_label: matchedTargetLabel,
    p_as_of_date: dateTo,
  });
  const opportunityAlert = opportunityAlertData?.[0] ?? null;
  ...
  const { data: daypartOpportunity } = await supabase.rpc("get_channel_daypart_opportunity", {
    p_channel_code: channel.code,
  ...
  // 그래서 이 두 섹션은 기간 선택과 무관하게 항상 dateTo 기준 최근 12주(84일) 고정 윈도우다.
  // 사용자 지시(2026-08-20): 4구간(새벽/오전/오후/저녁심야) 대신 3시간 단위 8구간으로 세분화.
  const { data: dowHourBlockPattern } = await supabase.rpc("get_channel_dow_hourblock_pattern", {
    p_channel_code: channel.code,
    p_program_target_label: programTargetLabel,
    p_as_of_date: dateTo,
    p_window_days: 84,
  });
  const { data: topPrograms } = await supabase.rpc("get_channel_top_programs", {
    p_channel_code: channel.code,
    p_program_target_label: programTargetLabel,
    p_as_of_date: dateTo,
    p_window_days: 84,
  ...
  const { data: periodDemographics } = await supabase.rpc("get_channel_period_demographics", {
    p_channel_code: channel.code,
  ...
  const { data: periodProgramMovers } = await supabase.rpc("get_channel_period_program_movers", {
    p_channel_code: channel.code,
    p_program_target_label: programTargetLabel,
    p_date_from: dateFrom,
    p_date_to: dateTo,
  ...
  // 오늘의 브리핑(줄글 보고서) — 사용자 지시: 최근 12주 평균 대비 요일별·시간대별 강세/약세와
  // 오늘 두드러진 지표를 종합. Page 1과 같은 함수(get_channel_daily_narrative)를 12주
  // baseline으로 재사용한다. dateTo 기준(원래도 "그 하루" 시점 서술이라 기간 선택과 무관).
  const { data: narrativeData } = await supabase.rpc("get_channel_daily_narrative", {
    p_channel_code: channel.code,
    p_target_label: matchedTargetLabel,
    p_program_target_label: programTargetLabel,
    p_demographic_labels: demographicTargets,
  ...
  const { data: compareChannelRow } = await supabase
    .from("channels")
    .select("primary_target")
    .eq("code", compareChannelCode)
    .maybeSingle();
