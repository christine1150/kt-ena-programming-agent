...
export async function GET(request: Request) {
  ...
  // 닐슨_채널시청률 올라올 때마다 자동으로 작성 및 배포".
  //
  // 사용자 재지시(2026-09-09, 트리거 교정): "주간 리뷰 섹션은 매 주 월요일(=일요일 일간 보고서가
  // 올라오는 날)에만 띄워줘" — 위 2026-09-07 지시("요일 무관, 매 요청마다 최신 주 반영")를
  // 철회하고, 주말 리포트(weekendReport)와 동일한 게이팅 규칙(asOfDateIsoDow===7, 즉 asOfDate가
  // 일요일 — 시청률이 하루 늦게 올라오는 이 서비스에서 "실제 월요일"에 해당)을 그대로 적용한다.
  // 다른 요일엔 계산 자체를 생략해 비용도 없앤다.
  let weeklyReview: {
    weekStart: string;
    weekEnd: string;
