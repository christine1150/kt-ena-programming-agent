...
export function normalizeProgramCanonicalName(name: string): string {
  ...
  //
  // ★ "스페셜"·"특별판"도 지우지 않는다(2026-09-19 사용자 확정: "스페셜은 별도 프로그램이 맞아").
  //   "지지고볶는여행"과 "지지고볶는여행스페셜", "백종원의레미제라블"과 "백종원의레미제라블특별판",
  //   "내아이의사생활"과 "내아이의사생활도아스페셜"은 따로 기획된 편성물이라 성과를 따로 집계한다.
  //   아래 규칙이 떼는 것은 같은 프로그램의 개별 방영분을 가리키는 꼬리표(회차·파트·방송 태그)뿐이다.
  //   "베스트"도 마찬가지다 — "기막힌이야기실제상황"과 "기막힌이야기실제상황베스트"는 별개다
  //   (2026-09-19 사용자 정정: "실제상황도 베스트랑 실제상황 나누자"). 예외는 두지 않는다.
  let s = ` ${name} `;
